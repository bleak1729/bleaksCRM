  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
        <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--txt2)', fontFamily: 'var(--fd)' }}>
          {label}
        </span>
        <span style={{
          fontSize: 13, fontWeight: 700, color: 'var(--ac)',
          fontFamily: 'var(--fm)', background: 'var(--ac-tint)',
          padding: '2px 8px', borderRadius: 'var(--r)', minWidth: 72, textAlign: 'right',
        }}>
          {fmt(value)}{unit}
        </span>
      </div>
      <div style={{ position: 'relative', height: 20, display: 'flex', alignItems: 'center' }}>
        ...
        <div style={{
          ...
          background: 'var(--ac)', borderRadius: 99, transition: 'width .1s',
        }} />
        <input
...
function ResultRow({ label, value, color, bold }: { label: string; value: string; color?: string; bold?: boolean }) {
  return (
    <div style={{
      ...
      padding: '6px 0', borderBottom: '1px solid var(--bor)',
    }}>
      <span style={{ fontSize: 12, color: 'var(--txt2)', fontFamily: 'var(--fd)', fontWeight: bold ? 700 : 500 }}>
        {label}
      </span>
      <span style={{
        fontSize: 13, fontWeight: bold ? 800 : 600,
        color: color ?? 'var(--txt)', fontFamily: 'var(--fm)',
      }}>
        {value}
      </span>
    </div>
...
function SectionCard({ title, icon, children }: { title: string; icon: string; children: React.ReactNode }) {
  return (
    <div style={{
      ...
      borderRadius: 'var(--r3)', padding: '20px 24px', boxShadow: 'var(--shadow)',
    }}>
      <h3 style={{
        display: 'flex', alignItems: 'center', gap: 8,
        fontSize: 12, fontWeight: 700, color: 'var(--txt)', fontFamily: 'var(--fd)',
        marginBottom: 20, textTransform: 'uppercase', letterSpacing: '.07em',
      }}>
...
export default function FreelanceCalculator() {
  ...
  const calc = useMemo(() => {
    ...
    // mensualizar sobre 4.33 semanas
    ...
    // proporciones para barra apilada (clamp a 0 para no dibujar negativo)
    ...
    return {
      horasTotales, baseTotal, ivaTotal, irpfTotal,
      baseMes, ivaMes, irpfMes, netoMes,
      totalGastosMes, margenLibre,
      pctGastos, pctAhorro, pctMargen, estado,
    }
  }, [
    tarifaHora, horasSemana, semanasProj, cuotaAut, irpfPct,
    alquiler, alimentacion, transporte, suministros, ocio, formacion, salud, gestoria,
    ahorroObj,
  ])
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '24px 16px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {/* Header */}
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 800, color: 'var(--txt)', fontFamily: 'var(--fd)', marginBottom: 4 }}>
            Calculadora Freelance
          </h2>
          <p style={{ fontSize: 13, color: 'var(--txt2)' }}>
            Fiscalidad española · IVA 21% · Cotización autónomos
          </p>
        </div>
        ...
        <SectionCard title="Proyecto / Ingresos" icon="💼">
          ...
          <div style={{
            marginTop: 8, background: 'var(--bg3)', borderRadius: 'var(--r2)',
            padding: '14px 16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 24px',
          }}>
          ...
          <div style={{ marginTop: 12 }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--txt3)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 6 }}>
              Desglose mensual
            </p>
        ...
        <SectionCard title="Gastos fijos mensuales" icon="🏠">
          ...
          <div style={{
            marginTop: 8, background: 'var(--danger-bg)', borderRadius: 'var(--r2)',
            padding: '10px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--danger)', fontFamily: 'var(--fd)' }}>
              Total gastos fijos / mes
            </span>
            <span style={{ fontSize: 15, fontWeight: 800, color: 'var(--danger)', fontFamily: 'var(--fm)' }}>
              {eur(calc.totalGastosMes)}
            </span>
          </div>
        </SectionCard>
        ...
        <div style={{
          ...
          borderRadius: 'var(--r3)', padding: '20px 24px', boxShadow: 'var(--shadow-md)',
        }}>
          <h3 style={{
            fontSize: 12, fontWeight: 700, color: 'var(--txt)', fontFamily: 'var(--fd)',
            marginBottom: 16, textTransform: 'uppercase', letterSpacing: '.07em',
          }}>
            📊 Resultado final
          </h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
            ...
            {/* Margen */}
            <div style={{
              background: calc.margenLibre >= 0 ? 'var(--success-bg)' : 'var(--danger-bg)',
              borderRadius: 'var(--r2)', padding: '14px 16px', textAlign: 'center',
            }}>
              <p style={{ fontSize: 11, color: calc.margenLibre >= 0 ? 'var(--success)' : 'var(--danger)', fontWeight: 600, marginBottom: 4, textTransform: 'uppercase', letterSpacing: '.05em' }}>
                Margen libre
              </p>
          ...
          <div style={{ marginBottom: 12 }}>
            <div style={{
              height: 24, borderRadius: 99, overflow: 'hidden',
              display: 'flex', background: 'var(--bg4)',
            }}>
              <div style={{
                width: `${calc.pctGastos}%`, background: '#dc2626',
                transition: 'width .3s ease', minWidth: calc.pctGastos > 0 ? 4 : 0,
              }} />
              <div style={{
                width: `${calc.pctAhorro}%`, background: '#d97706',
                transition: 'width .3s ease', minWidth: calc.pctAhorro > 0 ? 4 : 0,
              }} />
              <div style={{
                width: `${calc.pctMargen}%`, background: '#15803d',
                transition: 'width .3s ease', minWidth: calc.pctMargen > 0 ? 4 : 0,
              }} />
            </div>
            <div style={{ display: 'flex', gap: 16, marginTop: 8 }}>
              {[
                { label: 'Gastos', color: '#dc2626', pct: calc.pctGastos },
                { label: 'Ahorro', color: '#d97706', pct: calc.pctAhorro },
                { label: 'Margen', color: '#15803d', pct: calc.pctMargen },
              ].map(({ label, color, pct }) => (
          ...
          {/* Mensaje contextual */}
          <div style={{
            background: mensajeEstado.bg, borderRadius: 'var(--r2)',
            padding: '10px 14px', border: `1px solid ${mensajeEstado.color}22`,
          }}>
            <p style={{ fontSize: 12, fontWeight: 600, color: mensajeEstado.color, fontFamily: 'var(--fd)', lineHeight: 1.5 }}>
              {mensajeEstado.text}
            </p>
          </div>
        </div>
