...
// --- 2. Auditory feedback for XL/2XL buttons ---
...
export const useMaterialRipple = (disabled = false) => {
  ...
  const startPressAnimation = (event?: React.PointerEvent) => {
    ...
    growAnimationRef.current = rippleEffectRef.current.animate(
      ...
      { duration: PRESS_GROW_MS, easing: EASING_STANDARD, fill: ANIMATION_FILL }
    );
  };
...
// --- 6. MD3 Expressive Button Variants ---
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const innerContent = (
      <span className={cn("relative z-10 flex items-center justify-center gap-[inherit] pointer-events-none", contentClassName)}>
        {children}
      </span>
    );
